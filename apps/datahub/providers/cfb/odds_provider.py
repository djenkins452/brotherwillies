"""CFB Odds Provider — fetches odds from The Odds API."""
...
from apps.cfb.models import Game, OddsSnapshot, Team
...
class CFBOddsProvider(AbstractProvider):
    sport = 'cfb'
    data_type = 'odds'
    ...
    def fetch(self):
        """Fetch current CFB odds from The Odds API."""
        return self.client.get(
            f'/v4/sports/{CFB_SPORT_KEY}/odds/',
            params={
                'apiKey': self.api_key,
                'regions': 'us',
    ...
    def persist(self, normalized):
        ...
        for item in normalized:
            ...
            if commence:
                game = game.filter(
                    kickoff__date__gte=(commence - timedelta(days=1)).date(),
                    kickoff__date__lte=(commence + timedelta(days=1)).date(),
                )
            game = game.first()
